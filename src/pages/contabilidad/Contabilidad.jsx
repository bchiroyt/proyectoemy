      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 max-w-5xl w-full mx-auto">
        ...
        {datosContabilidad.map((item) => {
          ...
          return (
            ...
            >
              {/* Contenedor gris con hover que ahora maneja la redirección */}
              <div 
                onClick={() => navigate(item.ruta)}
                className="bg-gray-100 w-40 h-32 rounded-xl flex items-center justify-center mb-5 transition hover:shadow-[0_0_20px_var(--color-pagina)] cursor-pointer"
              >
              ...
              <h3 
                onClick={() => navigate(item.ruta)}
                className="text-lg font-semibold mb-2 cursor-pointer hover:text-(--color-pagina) transition-colors"
              >
                {item.titulo}
              </h3>
