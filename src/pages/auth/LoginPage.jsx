import React, { useState } from "react"; // ✅ useEffect eliminado
...
const LoginPage = () => {
  ...
  // Las validaciones ahora ocurren solo en eventos específicos
  ...
  const handleBlur = (e) => {
    ...
    // Validar el campo individualmente al salir del input
  ...
  const handleLogin = async (e) => {
    ...
    // Validar todo el formulario al intentar enviar
    const result = loginSchema.safeParse(formData);
  ...
  // Helper properties actualizadas para leer del estado
  ...
  return (
    <div className="flex min-h-screen h-screen w-full overflow-hidden bg-(--color-pagina-3)">

      {/* SECCION IZQUIERDA */}
      <div className="hidden lg:flex flex-col w-[45%] bg-(--color-pagina) p-10 justify-center items-center relative overflow-hidden rounded-[91%_9%_100%_0%/0%_100%_0%_100%]">
        <div className="flex flex-col items-center max-w-lg z-10 text-center">
          ...
          <div className="relative rounded-2xl overflow-hidden shadow-2xl border-2 border-(--color-blanco)/20 mb-8 w-full max-w-[360px] aspect-ratio: 5/3">
            <img src={logo1Img} alt="Estilo y Elegancia" className="w-full h-full object-cover" />
          </div>
